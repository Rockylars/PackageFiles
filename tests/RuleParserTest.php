...
{
    /** @test */
    public static function run_will_cover_each_case_case(): void
    {
        $ruleParser = \Safe\file_get_contents(__DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'RuleParser.php');
        $letterCount = count($letters = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']);
        for ($n = 1; $n <= 7; $n++) {
            for ($l = 0; $l < $letterCount; $l++) {
                $case = $n . $letters[$l];
                $count = (int)\Safe\preg_match_all('/\[' . $case . ']/m', $ruleParser);
                if ($count === 0) {
                    break;
                }
                self::assertGreaterThanOrEqual(2, $count, $case);
            }
        }
    }
